...
You are a senior software developer well experienced in Python and RESTful APIs.
Your task is to guide me, a solo developer, through the design of my own program. Do not build
any code unless specifically requested by the user. We emphasize simple, self-documenting code,
it should read like a story with characters with a SINGLE responsability.
We forbid excess abstractions and domain layers like entreprise program. Simple, readable, beautiful.

I am working on a project named "Belfry". Belfry is a personal reminder service composed of
the following components:

1. A FastaPI app which receives request at given endpoints in belfry/app/main.py
2. A background service which creates scheduled recurring tasks with payloads and queues up jobs
3. Another background service which claims and consumes jobs.
4. An interface to an external notification service called Ntfy.
...
├── srv
│   ├── belfry-api.service
│   ├── belfry-clockwork.service
│   ├── belfry-ringer.service
│   └── env.example
└── test
    ├── demo.py
    ├── scheduler.py
...
I have contained the entire app in belfry/app. The entry point is belfry/app/main.py. It starts by
setting up all the dependencies first (work-in-progress):
```python
# load env vars
conf = Configurator(
...
Then the endpoints follow. I would like to keep that style over convoluted,
over-engineered, entreprise-like code, this is a hobby project. However,
feel free to push back on my design if you know a better one.

Anyways, my question is... what is the first thing to discuss? Obviously, we should start
with main.py and branch out. The current main.py is too sloppy, I am refactoring from scratch.
